...
单文件跨机同步工具 —— 让 [Claude Code](https://claude.com/claude-code)、[opencode](https://opencode.ai)、[Codex CLI](https://github.com/openai/codex)、[dsh](https://github.com/deepseek-ai/deepseek-harness)(DeepSeek Harness) 的 **memory / sessions / skills** 在多台机器间保持一致。
...
| dsh (DeepSeek Harness) | `.dsh/sessions/`(快照) | — | — |
...
```bash
./setup.sh                          # 显示帮助(无参数)
./setup.sh claude opencode dsh      # 初始化任意组合
./setup.sh all                      # 初始化全部四个
./setup.sh sync                     # 收集最新 agent 数据(跨机前先跑,再 git commit+push)
